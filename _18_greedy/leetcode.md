# LeetCode Equivalent Problems (Greedy)
...
### **📊 Total LeetCode Questions: 15**

*Note: Greedy algorithms make the locally optimal choice at each step, hoping it leads to a globally optimal solution - this only works when the problem has the "greedy choice property" and "optimal substructure." Interviewers often pair a greedy solution with a proof sketch (usually an exchange argument) for why the local choice never hurts the global optimum, and contrast it with DP (e.g. Coin Change) where greedy fails for non-canonical denominations.*
